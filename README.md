...
Sphinx theme for [rising Docs](https://rising.readthedocs.io/en/latest/) based on the [Read the Docs Sphinx Theme](https://sphinx-rtd-theme.readthedocs.io/en/latest).
...
2. In your local docs or tutorials repo, remove any existing `rising_sphinx_theme` packages in the `src` folder (there should be a `pip-delete-this-directory.txt` file there)
3. In `requirements.txt` replace the existing git link with a link pointing to your commit or branch, e.g. `-e git+git://github.com/{ your repo }/rising_sphinx_theme.git@{ your commit hash }#egg=pt_lightning_sphinx_theme`
...
To be able to modify and preview the theme locally against the rising Docs and/or the PyTorch Lightning Tutorials first clone the repositories:

- [rising (Docs)](https://github.com/PhoenixDL/rising)
- [rising Tutorials](https://github.com/PhoenixDL/rising)
...
In `conf.py` change the html theme to `rising_sphinx_theme` and point the html theme path to this repo's local folder, which will end up looking something like:

```
html_theme = 'rising_sphinx_theme'
html_theme_path = ["../../../rising_sphinx_theme"]
```
